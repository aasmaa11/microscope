...
class ASIStageAxis:
    ...
    def _set_velocity(self, velocity_mmps): # tuple i.e. (2, 5, None)
        if self.serial_connection.verbose:
    ...
    def _get_velocity(self):
        ...
        self.velocity_mmps = self.serial_connection._send(
            'S '+'? '.join(self.name)+'?', parse_axes=True)
        if self.serial_connection.verbose:
            print("%s: -> velocity (mm/s) = %f"%(self.name, self.velocity_mmps))
    ...
    def _get_acceleration(self): # acceleration or deceleration ramp time (ms)
        ...
        self.acceleration_ms = self.serial_connection._send(
            'AC '+'? '.join(self.name)+'?', parse_axes=True)
        if self.serial_connection.verbose:
    ...
    def _get_settle_time(self): # time spent at end of move (ms)
        ...
        self.settle_time_ms = self.serial_connection._send(
            'WT '+'? '.join(self.name)+'?', parse_axes=True)
        if self.serial_connection.verbose:
    ...
    def _get_precision(self): # acceptable error between target and actual (um)
        ...
        precision_mm = self.serial_connection._send(
            'PC '+'? '.join(self.name)+'?', parse_axes=True)
        #self.precision_um = tuple(round(1e6 * p) for p in precision_mm)
        self.precision_um = round(1e6 * precision_mm)
        if self.serial_connection.verbose:
    ...
    def _counts2position(self, count):
        ...
        position_um = float(count) / self.encoder_counts_per_um
        return position_um
    ...
    def _finish_moving(self):
        ...
        assert self.position_um <= self._target_move_um + self.precision_um
        self._moving = False
        if self.serial_connection.verbose: print('%s: -> finished moving'%self.name)
        return None
    ...
    def move_um(self, move_um, relative=True, block=True):
        ...
        assert move_um >= self.min_position_um
        assert move_um <= self.max_position_um
...
class ASIStage:
    ...
    def __init__(self, 
        ...
        self.serial_connection = SerialConnection(which_port, name, verbose, very_verbose)
    ...
    # FUNCTIONS INHERITED FROM THE STAGE CLASS
    @property
